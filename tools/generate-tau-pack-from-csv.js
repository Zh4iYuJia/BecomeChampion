...
const OUT_JSON_PATH = path.join(ROOT, 'rulepacks', 'TAU-rules-pack.json');

const PACK_VERSION = '1.0.0';
const PACK_ID = 'tau-rules-pack';
const PACK_NAME = 'TAU-rules-pack.json';
const CORE_STRATAGEM_TYPE_RE = /^Core\s*[–-]/i;

const INCLUDED_DETACHMENTS = [
  { id: '000000781', name: "Kauyon" },
  { id: '000000845', name: "Mont'ka" },
  { id: '000000846', name: 'Retaliation Cadre' },
  { id: '000000847', name: 'Kroot Hunting Pack' },
  { id: '000001005', name: 'Auxiliary Cadre' },
  { id: '000001028', name: 'Experimental Prototype Cadre' },
];
...
function isTauSource(sourceRow) {
  return /t[\u2019']au empire/i.test(String(sourceRow?.name || ''));
}

function isTauUnit(datasheet, sourceRow) {
  if (!datasheet || datasheet.faction_id !== 'TAU') return false;
  return isTauSource(sourceRow);
}
...
function buildUnits(datasheets, abilityMap, keywordMap, sourceMap) {
  ...
  for (const datasheet of datasheets) {
    ...
    if (!isTauUnit(datasheet, sourceRow)) continue;
...
function main() {
  ...
  const pack = {
    ...
    author: 'Community',
    source_note: `Generated from current 10e CSV exports. This pack includes T'au Empire detachments ${detachmentNames.join(', ')} plus standard Core Stratagems. Boarding Actions detachments are intentionally excluded. Rules text is extracted and normalized for in-app prompting; check official publications for authoritative wording.`,
    units,
    stratagems,
