...
const OUT_JSON_PATH = path.join(ROOT, 'rulepacks', 'TYR-rules-pack.json');

const PACK_VERSION = '1.0.0';
const PACK_ID = 'tyr-rules-pack';
const PACK_NAME = 'TYR-rules-pack.json';
const CORE_STRATAGEM_TYPE_RE = /^Core\s*[–-]/i;

const INCLUDED_DETACHMENTS = [
  { id: '000000748', name: 'Invasion Fleet' },
  { id: '000000769', name: 'Crusher Stampede' },
  { id: '000000770', name: 'Unending Swarm' },
  { id: '000000771', name: 'Assimilation Swarm' },
  { id: '000000772', name: 'Vanguard Onslaught' },
  { id: '000000773', name: 'Synaptic Nexus' },
];
...
function isTyranidSource(sourceRow) {
  return /tyranid/i.test(String(sourceRow?.name || ''));
}

function isTyranidUnit(datasheet, sourceRow) {
  if (!datasheet || datasheet.faction_id !== 'TYR') return false;
  return isTyranidSource(sourceRow);
}
...
function buildUnits(datasheets, abilityMap, keywordMap, sourceMap) {
  ...
  for (const datasheet of datasheets) {
    ...
    if (!isTyranidUnit(datasheet, sourceRow)) continue;
...
function main() {
  ...
  const pack = {
    ...
    author: 'Community',
    source_note: `Generated from current 10e CSV exports. This pack includes Tyranid detachments ${detachmentNames.join(', ')} plus standard Core Stratagems. Boarding Actions detachments are intentionally excluded. Rules text is extracted and normalized for in-app prompting; check official publications for authoritative wording.`,
    units,
    stratagems,
