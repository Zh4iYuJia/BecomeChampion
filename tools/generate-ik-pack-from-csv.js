...
const OUT_JSON_PATH = path.join(ROOT, 'rulepacks', 'IK-rules-pack.json');

const PACK_VERSION = '1.0.0';
const PACK_ID = 'ik-rules-pack';
const PACK_NAME = 'IK-rules-pack.json';
const CORE_STRATAGEM_TYPE_RE = /^Core\s*[–-]/i;

const INCLUDED_DETACHMENTS = [
  { id: '000000988', name: 'Questor Forgepact' },
  { id: '000001106', name: 'Valourstrike Lance' },
  { id: '000001107', name: 'Gate Warden Lance' },
  { id: '000001108', name: 'Questoris Companions' },
  { id: '000001109', name: 'Spearhead-At-Arms' },
];
...
function isImperialKnightsSource(sourceRow) {
  return /imperial knights/i.test(String(sourceRow?.name || ''));
}

function isImperialKnightsUnit(datasheet, sourceRow) {
  if (!datasheet || datasheet.faction_id !== 'QI') return false;
  return isImperialKnightsSource(sourceRow);
}
...
function buildUnits(datasheets, abilityMap, keywordMap, sourceMap) {
  ...
  for (const datasheet of datasheets) {
    ...
    if (!isImperialKnightsUnit(datasheet, sourceRow)) continue;
...
function main() {
  ...
  const pack = {
    ...
    author: 'Community',
    source_note: `Generated from current 10e CSV exports. This pack includes Imperial Knights detachments ${detachmentNames.join(', ')} plus standard Core Stratagems. Rules text is extracted and normalized for in-app prompting; check official publications for authoritative wording.`,
    units,
    stratagems,
